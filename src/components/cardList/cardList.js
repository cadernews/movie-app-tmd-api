...
export default class CardList extends Component {
  ...
  componentDidUpdate(prevProps) {
    ...
    if (guestSessionId !== prevProps.guestSessionId) {
      this.moviesServise
        .getAllMoviesRatingGuestSession(guestSessionId)
        .then((res) => {
  ...
  getMovies = () => {
    ...
    if (searchQuery) {
      this.moviesServise
        .searchMovie(pageNumber, searchQuery)
        .then(this.onMoviesLoaded)
        .catch(this.onError);
    } else {
      this.moviesServise
        .getPopularMovie(pageNumber)
        .then(this.onMoviesLoaded)
        .catch(this.onError);
    }
  };
  ...
  movieRatingHandler = (movieId, rating) => {
    ...
    this.moviesServise
      .setRatingGuest(guestSessionId, movieId, rating)
      .catch(() => {
  ...
  render() {
    ...
    switch (status) {
      ...
      case "norated":
        content = (
          <Space size="middle" direction="vertical" align="center">
            <Alert
              message="Can't find rated movies"
              description="Find and rate some!"
              type="warning"
              showIcon
            />
            <Button
              size="large"
              type="primary"
              onClick={() => selectedTabHandler("search")}
            >
              To search!
            </Button>
          </Space>
      ...
      default:
        content = (
          <Row gutter={[{ md: 32 }, { xs: 20, sm: 24, md: 32 }]}>
